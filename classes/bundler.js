...
export class Bundler {
  ...
  constructor(arg1) {
    if (arg1 instanceof Loader) this.#loader = arg1;
      ...
      throw new Error(
        "Invalid first argument. Expected Loader or CLI instance."
      );
  ...
  async run(filename, options = {}) {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
      ...
      options.define[`process.env.${keys[i]}`] = JSON.stringify(
        process.env[keys[i]]
      );
    }
  ...
  return {
    ...
    setup: (build) => (
      build.onResolve({ filter }, async ({ path, importer, resolveDir }) => {
        ...
        if (!fs.isAbsolute(importer))
          importer = fs.resolve(resolveDir, importer);
      ...
      build.onLoad(
        { filter },
        async ({ path, namespace, with: importAttributes = {} }) => {
          if (namespace === "ignore") return { contents: "" };
          if (namespace === "window")
            return { contents: `module.exports = window.${path};` };
          if (namespace === "global")
            return { contents: `module.exports = global.${path};` };
          ...
          const {
            ...
            resources,
            files = [],
          } = await loader.load(path, { importAttributes }, () => ({}));
